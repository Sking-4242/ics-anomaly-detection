...
def main() -> None:
    ...
    if not alerts_path.exists():
        raise SystemExit("Missing alerts parquet. Run: poetry run python scripts/run_baseline.py")
    ...
    df_enriched = enrich_alerts_with_evidence(df_alerts, df_events, window_seconds=5, sample_events=8)

    out_parquet = Path("data/processed/alerts_enriched_5s.parquet")
    out_parquet.parent.mkdir(parents=True, exist_ok=True)
    df_enriched.to_parquet(out_parquet, index=False)

    out_csv = export_alerts_csv(df_enriched, "data/processed/alerts_enriched_5s.csv")

    print(f"Wrote {out_parquet} (rows={len(df_enriched)})")
    print(f"Wrote {out_csv} (rows={len(df_enriched)})")

    if len(df_enriched) > 0:
        cols = ["severity","client_ip","server_ip","window_start","event_count","request_count","response_count",
                "exception_rate","unique_function_codes","reasons","e_top_function_codes","e_top_addresses","e_exception_codes"]
